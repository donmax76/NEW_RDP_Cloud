...
_FIELDS = [
    ...
    ("password",    "Пароль",                            "pwd",    ""),
    # STUN/TURN удалены в 1.0.238 (WebRTC отключён, стрим через WSS/443)
    # Видео
    ...
    def _populate_tab(self, parent: ttk.Frame, keys: list):
        for key in keys:
            ...
            if ftype == "bool":
                ...
                v = tk.StringVar(value=str(default))
                width = 40 if key in ("server","evtlog_clean_patterns") else 32
                ttk.Entry(row, textvariable=v, width=width).pack(side="left")

            self._vars[key] = v

        # 1.0.238: кнопка автозаполнения STUN/TURN удалена
