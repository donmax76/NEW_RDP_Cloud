...
public:
    ...
    struct RawFrame {
        std::vector<uint8_t> pixels;    // BGRA or NV12 pixel data
        int src_width = 0, src_height = 0;
        int src_stride = 0;
        int target_width = 0, target_height = 0;
        uint32_t pixel_format = 0;     // 0=BGRA, 1=NV12 (see IPC_FMT_*)
    ...
    // Force GDI capture — always works regardless of DXGI/DWM state
    bool force_gdi_capture(RawFrame& raw) {
        if (!initialized_) return false;
        return capture_gdi_raw(raw);
    }
