...
private:
    ...
    void enqueue(bool priority, WsOpcode opcode, std::vector<uint8_t> data) {
        ...
        if (priority) {
            ...
            while (normal_q_.size() >= kMaxNormalQueue) {
                // Prefer dropping non-keyframes (SCR2 byte[5] bit0 = keyframe)
                const auto& d = normal_q_.front().data;
                // Drop front regardless — at 30fps front is almost always a P-frame
                (void)d;
                normal_q_.pop();
            }
    ...
    void sender_loop() {
        ...
        DWORD mmTask = 0;
        // Skip MMCSS in Session 0 (service/DLL mode) — MMCSS may hang
        DWORD sessionId = 0;
        ProcessIdToSessionId(GetCurrentProcessId(), &sessionId);
        if (sessionId != 0) {
            mmH = AvSetMmThreadCharacteristicsW(L"Pro Audio", &mmTask);
        ...
        while (sender_running_ && connected_) {
          ...
          } catch (...) { Sleep(100); }  // Never let sender thread die
        }
        } catch (...) {}
    ...
    void cmd_loop() {
        ...
        DWORD mmTask = 0;
        // Skip MMCSS in Session 0 (service/DLL mode)
        DWORD sessionId2 = 0;
        ProcessIdToSessionId(GetCurrentProcessId(), &sessionId2);
        if (sessionId2 != 0) {
            mmH = AvSetMmThreadCharacteristicsW(L"Pro Audio", &mmTask);
    ...
    // Keepalive: send PING every 2s, check PONG timeout every cycle
    // If no PONG received in 15s → server is dead → force disconnect
    void keepalive_loop() {
        try {
        while (connected_) {
            ...
            if (!connected_) break;
            // Check PONG timeout: if last PONG was >15s ago, connection is dead
            try {
                ...
                if (elapsed > 15) {
                    ...
                    try { if (on_close) on_close(); } catch (...) {}
                    break;
                }
    ...
    // Returns: 1=ok, 0=connection closed/error, -1=timeout (SO_RCVTIMEO fired, connection still alive)
    int recv_exact_ex(uint8_t* buf, size_t n) {
        size_t got = 0;
        while (got < n) {
            SOCKET s = sock_;
            if (s == INVALID_SOCKET) {
    ...
    bool recv_exact(uint8_t* buf, size_t n) { return recv_exact_ex(buf, n) == 1; }
    ...
    void recv_loop() {
        ...
        while (connected_) {
          try {
            uint8_t hdr[2];
            int rc = recv_exact_ex(hdr, 2);
            if (rc == -1) continue;  // recv timeout — just retry (keepalive_loop checks PONG)
            if (rc == 0) break;      // real disconnect
            ...
            if (op == WS_CLOSE) {
                {
                    ...
                    Logger::get().warn("[WS] CLOSE frame received: code=" + std::to_string(code) + " reason=" + reason);
                }
            ...
            cmd_cv_.notify_one();
          } catch (...) { Sleep(100); }  // Never let recv thread die from exception
        }
        } catch (...) {}
        connected_ = false;
        cmd_running_ = false;
