...
// MinGW workaround: __uuidof(ICodecAPI) not defined in mingw headers
#if defined(__MINGW32__) || defined(__MINGW64__)
#include <initguid.h>
DEFINE_GUID(MINGW_IID_ICodecAPI, 0x901db4c7, 0x31ce, 0x41a2, 0x85, 0xdc, 0x8f, 0xa0, 0xbf, 0x41, 0xb8, 0xda);
template<> inline const GUID& __mingw_uuidof<ICodecAPI>() { return MINGW_IID_ICodecAPI; }
#endif
...
public:
    ...
    // Encode pre-converted NV12 data (skip BGRA→NV12 conversion)
    EncodedFrame encode_nv12(const uint8_t* nv12_data, int w, int h, bool request_keyframe = false) {
        EncodedFrame result;
        if (!initialized_ || !encoder_) return result;
        if (w != width_ || h != height_) return result; // dimension mismatch

        if (request_keyframe) force_keyframe();

        UINT32 buf_size = w * h * 3 / 2;
        ...
        memcpy(buf_ptr, nv12_data, buf_size);
        frame_buf->SetCurrentLength(buf_size);
        frame_buf->Unlock();
        ...
        if (hr == MF_E_NOTACCEPTING) {
            ...
            if (!extra.data.empty()) {
                ...
                else result.data.insert(result.data.end(), extra.data.begin(), extra.data.end());
            }
            hr = encoder_->ProcessInput(0, sample.Get(), 0);
        }
        if (SUCCEEDED(hr)) {
            ...
            if (!post.data.empty()) {
                if (result.data.empty()) result = std::move(post);
                else {
                    result.data.insert(result.data.end(), post.data.begin(), post.data.end());
                    result.is_keyframe = result.is_keyframe || post.is_keyframe;
                }
            }
        }
        return result;
